...
public class Project {
    ...
    public Project(String name, int requiredWork, double reward, boolean strategic) {
        if (name == null || name.isBlank()) throw new IllegalArgumentException("Project name cannot be blank.");
        if (requiredWork <= 0)              throw new IllegalArgumentException("Required work must be positive.");
    ...
    public void addEmployee(Employee employee) {
        ...
        if (!team.contains(employee)) team.add(employee);
    }
    ...
    public void workOneTurn() {
        if (status != ProjectStatus.IN_PROGRESS || isFinished()) return;
        for (Employee employee : team) progress += employee.work();
        if (progress >= requiredWork) { progress = requiredWork; status = ProjectStatus.FINISHED; }
    }

    public boolean isFinished()          { return progress >= requiredWork; }
